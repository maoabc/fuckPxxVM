...
public final class ApkUtils {
     ...
     * Reads all files from an input stream that is reading from a zip file.
     */
    public static void getFiles(InputStream inputStream, Pattern regex, Extract extract)
            throws IOException {
        try (ZipInputStream zipInputStream = new ZipInputStream(inputStream);
             BufferedInputStream bis = new BufferedInputStream(zipInputStream)) {
            ZipEntry entry;
            while ((entry = zipInputStream.getNextEntry()) != null) {
                if (regex.matcher(entry.getName()).matches()) {
                    extract.one(entry.getName(), toByteArray(bis));
                }
                zipInputStream.closeEntry();
            }
        }
    ...
    public interface Extract {
        void one(String entryName, byte[] data);
    }
}
